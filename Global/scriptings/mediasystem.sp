...
public void OnClientCookiesCached(int client)
{
	...
	g_iVolume[client] = (StringToInt(buf[1]) >= 10) ? StringToInt(buf[1]) : 80;
...
public int API_SearchMusic(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, int client)
{
	if(!bFailure && bRequestSuccessful && eStatusCode == k_EHTTPStatusCode200OK)
		SteamWorks_GetHTTPResponseBodyCallback(hRequest, API_SearchResult, client);
	else
...
public int API_SearchResult(const char[] sData, int client)
{
	...
	ProcessElement("MusicData", Response, hObj);
...
void InitPlayer(int client)
{
	...
	int target_list[MAXPLAYERS+1], target_count=0;
	...
	{
		if(g_bDiable[target_list[i]])
			continue;

		char murl[192];
		Format(murl, 192, "%s%d&volume=%d", PLAYER, g_Sound[iSongId], g_iVolume[target_list[i]])
		DisplayMenu(g_hPlayMenu, target_list[i], 15);
...
public int API_GetLyric(Handle hRequest, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, int client)
{
	...
	{
		char lyricTxt[128];
		BuildPath(Path_SM, lyricTxt, 128, "data/lyric.txt");
		if(SteamWorks_WriteHTTPResponseBodyToFile(hRequest, lyricTxt))
		{
			Handle hFile;
			if((hFile = OpenFile(lyricTxt, "r")) != INVALID_HANDLE)
			{
				LyricHud("....等待歌词中....");
				...
				while(ReadFileLine(hFile, fileline, 128))
				{
					...
					Handle timer = CreateTimer(StringToFloat(time[0])*60.0+StringToFloat(time[1]), Timer_Lyric, PushArrayString(array_lyric, data[1]), TIMER_FLAG_NO_MAPCHANGE);
...
public void ProcessElement(char[] sKey, KeyValues g_hKV, JSONValue hObj)
{
...
public void IterateJsonArray(JSONArray hArray, KeyValues g_hKV)
{
	for(int iElement = 0; iElement < json_array_size(hArray); iElement++)
	{
		...
		ProcessElement(sElement, g_hKV, hValue);

		CloseHandle(hValue);
	}
}

public void IterateJsonObject(JSONObject hObj, KeyValues g_hKV)
{
	...
	while(hIterator != INVALID_HANDLE)
	{
		...
		ProcessElement(sKey, g_hKV, hValue);
...
void LyricHud(const char[] message)
{
